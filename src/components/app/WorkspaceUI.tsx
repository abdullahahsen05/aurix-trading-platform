      ...
      <table className="w-full min-w-[780px] text-left text-sm">
        <thead className="bg-panel-strong text-[11px] font-semibold uppercase tracking-[0.1em] text-muted">
          <tr>
            {headers.map((header, index) => (
              <th
                key={header}
                className={`h-11 px-4 py-3 ${index > 0 ? "text-right" : ""}`}
              >
                {header}
              </th>
            ))}
        ...
        <tbody>
          {rows.map((row, rowIndex) => (
            <tr
              key={rowIndex}
              className="h-12 border-t border-line/80 transition-colors hover:bg-white/[0.025]"
            >
              {row.map((cell, cellIndex) => (
                <td key={cellIndex} className={`px-4 py-3 text-foreground/80 ${cellIndex === 0 ? "font-medium text-foreground" : "text-right tabular-nums"}`}>
                  {cell}
                </td>
              ))}
            </tr>
