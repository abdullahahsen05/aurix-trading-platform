...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return jsonOk({ withdrawal: await transitionPartnerWithdrawal({ withdrawalId: id, adminId: admin.id, nextStatus: "APPROVED", adminNote: body.adminNote }) });
  } catch (error) {
    if (error instanceof AuthError) return jsonFail(error.code, error.message, error.statusCode);
    return jsonFail("WITHDRAWAL_ACTION_FAILED", error instanceof Error ? error.message : "Approval failed", 400);
  }
}
